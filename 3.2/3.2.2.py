...
if __name__ == "__main__":
    #создаём методы для сортировки

    def sort_area_dict(dict):
        sort = sorted(dict.items(), key=lambda item: item[1], reverse=True)[:10]
    ...
    #класс, отвечающий за ввод данных
    class InputConnect:
        def __init__(self):
            self.file_name = input("Введите название файла: ")
            self.vacancy_name = input("Введите название профессии: ")

    #метод переформировывает данные в csv формат
    class MakeCsv:
    ...
    #получаем все необходимые данные. формируем по ним csv
    ...
    salaries_by_year, vacancies_by_year, inp_vacancy_salary, inp_vacancy_count, salaries_areas, vacancies_areas \
        = {}, {}, {}, {}, {}, {}
    #в циклах формируем статистики и распределяем по процессам
    for city in cities:
    ...
    processes = []
    for year in years:
        #создаём отдельный процесс для разделения работы
        p = Process(target=start, args=(vac, year, q))
        processes.append(p)
        p.start()
